...
export class AppComponent {
  ...
  getText(event: any) {
    ...
    ELEMENT_DATA.forEach((item) => {
      if (item.name.indexOf((event.target as HTMLInputElement).value) != -1) {
        tidyData.push(item);
      }
    })
  ...
  // 上課方法
  // getText() {
  //   console.log('取得的文字', this.immediateInput);
  //   const filterData = this.immediateInput.trim().toLowerCase();
